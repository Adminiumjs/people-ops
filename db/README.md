...
`docker compose up` gives you a working product: `hr-db` creates the
...
By default these talk to the `hr-db` container through
...
```bash
DATABASE_URL=postgres://user:pass@host:5432/foundry npm run demo:status
```
